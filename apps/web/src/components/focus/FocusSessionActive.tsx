/**
 * FocusSessionActive - Active focus session UI that uses global FocusTimerContext
 * 
 * This component reads timer state from the global context and renders the session UI.
 * The timer continues even when navigating away from this page.
 */
...
interface FocusSessionActiveProps {
  duration: number; // in minutes (original duration)
  purpose: string;
  linkedTaskId?: string;
  onComplete: (endedEarly: boolean, actualMinutes: number) => void;
  onCancel: () => void;
}

// Simple focus session orb visual
function FocusSessionOrb({ progress, isPaused }: { progress: number; isPaused: boolean }) {
...
export function FocusSessionActive({ duration, purpose, linkedTaskId, onComplete, onCancel }: FocusSessionActiveProps) {
  const { 
    isRunning,
    isPaused, 
    isCompleted,
  ...
  // Handle completion - wait for celebration to be dismissed, then call onComplete
  useEffect(() => {
    if (isCompleted && !hasTriggeredComplete && !showCelebration) {
      // Celebration was shown and dismissed
      setHasTriggeredComplete(true);
      console.log('[FocusSessionActive] Timer completed, celebration dismissed');
      onComplete(false, duration);
    }
  }, [isCompleted, showCelebration, hasTriggeredComplete, onComplete, duration]);
  ...
  }, [isRunning, isPaused, pauseFocus, resumeFocus]);

  const handleEndEarly = useCallback(() => {
    const elapsedSeconds = duration * 60 - remaining;
    const actualMinutes = Math.max(1, Math.floor(elapsedSeconds / 60));
    endFocusEarly();
    onComplete(true, actualMinutes);
  }, [remaining, duration, endFocusEarly, onComplete]);
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center">
        ...
        {/* Focus Session Orb with progress ring */}
        <div className="relative">
          <FocusSessionOrb progress={progress} isPaused={isPaused} />
