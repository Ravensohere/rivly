...
// Simple focus session orb visual (not the main RhythmOrb)
...
export function FocusSession({ duration, purpose, linkedTaskId, onComplete, onCancel }: FocusSessionProps) {
  const { 
    isRunning,
    isPaused, 
    isCompleted,
  ...
  const { showCelebration, dismissCelebration } = useTimerCompletion();
  ...
  useEffect(() => {
    if (!hasStarted) {
      start(duration * 60, purpose);
      setHasStarted(true);
      console.log('[FocusSession] Started background timer for', duration, 'minutes');
    }
  }, [hasStarted, start, duration, purpose]);
  ...
  useEffect(() => {
    if (isCompleted && !hasTriggeredComplete && !showCelebration) {
      ...
      console.log('[FocusSession] Timer completed, celebration dismissed');
  ...
  }, [remaining, duration, cancel, onComplete]);
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center">
        ...
        >
          ...
          <span>Timer continues in background (best effort)</span>
        </motion.div>
